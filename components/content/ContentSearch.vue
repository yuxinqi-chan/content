            ...
            link.to && {
              id:
                typeof link.to === "string" && link.to.startsWith("http")
                  ? link.to
                  : router.resolve(link.to).fullPath,
              ...link,
              icon: link.icon || ui.value.fileIcon.name,
            },
            ...(link.children || []).map(
              (child: { to: string; description: any; icon: any }) => {
                return {
                  id:
                    typeof child.to === "string" && child.to.startsWith("http")
                      ? child.to
  ...
  return [
    {
      ...
      children: concatChildren(
        extractUntilFirstTitle(file?.body?.children || []),
      ),
      icon: fileIcon(file),
    },
...
function remapChildren(children: MarkdownNode[]) {
  return (
    (children
      ?.map((grandChild) => {
        if (
          ["code", "code-inline", "em", "a", "strong"].includes(
            grandChild.tag as string,
          )
        ) {
          return {
            type: "text",
            value:
              grandChild.children?.find((child) => child.type === "text")
                ?.value || "",
          };
  ...
  return children
    .map((child) => {
      ...
      if (
        ...
        // @ts-ignore
        grandChildren = remapChildren(grandChildren).reduce(
          (acc: MarkdownNode[], grandChild) => {
            // @ts-ignore
...
function groupByHeading(children: MarkdownNode[]) {
  const groups: Record<string, { title?: string; children: MarkdownNode[] }> =
    {}; // grouped by path
        ...
        node.children
          ?.map((child) => {
            ...
            if (
              ...
              return child.children?.find((child) => child.type === "text")
                ?.value;
            }
          })
          ?.filter(Boolean)
