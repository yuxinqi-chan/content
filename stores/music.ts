...
import type { SongType, LyricType } from "@/types/player";
...
export const useMusicStore = defineStore({
  ...
  getters: {
    ...
    isHasLrc(state): boolean {
      return (
        state.songLyric.lrcData.length > 0 && state.playSong.type !== "radio"
      );
    },
    // 是否具有逐字歌词
    ...
    personalFMSong(state): SongType {
      return (
        state.personalFM.list?.[state.personalFM.playIndex] || defaultMusicData
      );
    },
  },
  actions: {
